...
class Snapshot(QDialog):
    def __init__(self, parent=None, f=Qt.Tool | Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint | Qt.X11BypassWindowManagerHint):
        ...
        self.isFullScreen()
        self.desktopGeometry = util_qt.GetDesktopGeometry(app)
    ...
    def takeScreenshot(self):
        ...
        self.shotfilename = os.path.join(util_qt.GetWorkingPath(), "temp", time.strftime('%Y%m%d-%H%M%S')) + ".png"
    ...
    def openTranslator(self):
        ...
        t = threading.Thread(target=util_qt.WebServer, args=(self.address, self.port))
        t.daemon = True
        t.start()

        print("app framesize: " + str(util_qt.GetAppFrameSize(self.screenshot).width()) + "x" + str(util_qt.GetAppFrameSize(self.screenshot).height()))
        ...
        self.view.setWindowIcon(QIcon(os.path.join(util_qt.GetWorkingPath(), "img", "app-icon.ico")))
        ...
        self.view.resize(util_qt.GetAppFrameSize(self.screenshot))
        self.view.show()
...
if __name__ == '__main__':
    atexit.register(util_qt.Cleanup)
